    ...
    <x-form>
        ...
        <div class="grid grid-cols-1 md:grid-cols-5 gap-2">
            @foreach ($inventories as $key => $inventory)
                <div class="col-span-5 flex justify-start space-x-2 items-center">
                    <x-select label="Material {{$loop->iteration}}" :options="$materials" option-value="id" option-label="material_name" placeholder-value="" placeholder="Select Material" inline wire:model.live="inventories.{{$key}}.material_id" />
                ...
                @if (isset($inventory['material_id']) && !empty($inventory['material_id']))
                    <div class="col-span-5 py-1"></div>
                    <x-input type="number" label="Price per Kgs" inline wire:model.live="inventories.{{$key}}.price_per_kilograms" />
                    <x-input type="number" label="Inventory Cost" inline wire:model.live="inventories.{{$key}}.inventory_cost" />
                    <x-input type="number" label="Kgs per bag"  inline wire:model.live="inventories.{{$key}}.kilograms_per_bag" />
                    <x-input type="number" label="Deliveries Today"  inline wire:model.live="inventories.{{$key}}.deliveries_today" />
                    <x-input type="number" label="Standard day" inline wire:model.live="inventories.{{$key}}.standard_days" />
                @endif
                <hr class="my-1 col-span-5">
            @endforeach
        </div>
